...
/// The six `qubit-retry` 0.22.0 terminals preserve their exact details without
...
pub enum CasRetryFailure {
    ...
    /// A terminal variant outside the exact `qubit-retry` 0.22.0 contract.
    ///
    /// This defensive classification can only be observed when a substituted
    /// path source keeps the 0.22.0 package metadata while extending its
