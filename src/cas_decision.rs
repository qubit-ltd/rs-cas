...
impl<T, R, E> CasDecision<T, R, E> {
    ...
    ///
    /// # Parameters
    /// - `next`: Replacement state allocation to publish after a successful
    ///   compare-and-swap.
    /// - `output`: Business output returned on success.
    ///
    /// # Returns
    /// A [`CasDecision::Update`] value retaining the supplied allocation. The
    /// executor compares its observed snapshot with the atomic current value
    /// by pointer identity, not `T` value equality. Reusing an earlier `Arc`
    /// can therefore be affected by A-B-A changes; do not mutate the pointee
    /// through interior mutability while it is a shared CAS snapshot.
    #[inline(always)]
