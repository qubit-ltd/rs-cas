...
fn test_report_exposes_counts_ratios_and_limits() {
    ...
    assert_eq!(report.max_total_elapsed(), Some(Duration::from_secs(3)));
...
fn test_report_contention_threshold_matching() {
    ...
    assert!(!report.is_contention_hot(&ContentionThresholds::new(2, 2, 0.5)));
    assert!(!report.is_contention_hot(&ContentionThresholds::new(2, 1, 0.75)));
}

/// A no-write default execution reports no failures or soft budgets.
#[test]
fn test_default_finish_report_preserves_unbounded_budgets() {
    let state = AtomicRef::from_value(1usize);
    let outcome = CasExecutor::<usize, TestError>::builder()
        .build()
        .expect("default executor")
        .execute(&state, |_: &usize| CasDecision::finish("unchanged"));
    ...
    assert_eq!(report.aborts(), 0);
    assert_eq!(report.timeouts(), 0);
    assert_eq!(report.max_attempts(), DEFAULT_CAS_MAX_ATTEMPTS);
    assert_eq!(report.max_operation_elapsed(), None);
    ...
    assert_eq!(report.retryable_failure_ratio(), 0.0);
    assert!(report.is_contention_hot(&ContentionThresholds::new(1, 0, 0.0)));
    assert!(report.finished_at() >= report.started_at());
    assert_eq!(
        report.elapsed(),
        report.finished_at().duration_since(report.started_at())
    ...
    assert_eq!(outcome.expect("no-write finish").into_output(), "unchanged");
